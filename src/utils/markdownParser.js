...
function highlightLine(rest, lang) {
    let out;
    try {
        out =
    ...
    return out.replace(/class="hljs-([a-z_]+)[^"]*"/g, (_m, c) => HL_COLORS[c] ? `style="color:${HL_COLORS[c]}"` : '');
}
 ...
 */
export function collectFormulas(md) {
    const seen = new Set();
    const result = [];
 ...
 * 批量预渲染公式为 SVG。
 */
export async function preRenderFormulas(formulas) {
    const map = new Map();
    const results = await Promise.all(formulas.map(async (f) => {
 ...
 * 推荐所有 caller 使用这个入口。
 */
export async function parseMarkdownAsync(md, t) {
...
export function parseMarkdown(md, t, formulaMap) {
    ...
    while (i < lines.length) {
        ...
        // 代码块：微信 code-snippet 原生渲染，code 内走纯文本（不 highlight 多 span），空格转 &nbsp;
        if (/^```/.test(line)) {
            ...
            for (const ln of codeLines) {
                ...
                // 纯文本：不 highlight，空格转 &nbsp; 防止微信吞空格
                const text = esc(rest).replace(/ /g, '&nbsp;');
        ...
        if (line.indexOf('|') >= 0 && i + 1 < lines.length && /\|[\s-:]+\|/.test(lines[i + 1])) {
            ...
            while (i < lines.length && lines[i].indexOf('|') >= 0 && lines[i].trim() !== '') {
                ...
                if (cellLine.startsWith('|'))
                    cellLine = cellLine.slice(1);
                if (cellLine.endsWith('|'))
                    cellLine = cellLine.slice(0, -1);
